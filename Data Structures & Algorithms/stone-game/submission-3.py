    ...
    def stoneGame(self, piles: List[int]) -> bool:
        ...
        def r(alice_score, turn, left_idx, right_idx):
            ...
            if q[left_idx] > q[right_idx]:
                ...
                if turn:
                    res = r(q[left_idx] + alice_score, not turn, left_idx, right_idx-1)
                else:
                    res = r(alice_score, not turn, left_idx, right_idx-1)
            else:
                if turn:
                    res = max(r(q[left_idx] + alice_score, not turn, left_idx+1, right_idx), r(q[0] + alice_score, not turn, left_idx, right_idx-1))
                else:
