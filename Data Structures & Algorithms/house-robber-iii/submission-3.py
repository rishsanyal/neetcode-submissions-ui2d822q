...
class Solution:
    def rob(self, root: Optional[TreeNode]) -> int:
        ...
        def traverse(node, can_select=True):
            ...
            if (node, can_select) in cache:
                return cache[(node, can_select)]

            if can_select:
                options = max(
                    node.val + traverse(node.left, False) + traverse(node.right, False),
                    traverse(node.left, True) + traverse(node.right, True)
                )
            else:
                options = traverse(node.left, True) + traverse(node.right, True)

            cache[(node, can_select)] = options
            res = max(res, options)
