...
class Solution:
    def insert(self, intervals: List[List[int]], newInterval: List[int]) -> List[List[int]]:
        ...
        for idx in range(len(intervals)):
            ...
            if newInterval and ((curr_start <= newInterval[0] <= curr_end) or (curr_start <= newInterval[1] <= curr_end) or (newInterval[0] <= curr_start <= newInterval[1]) or (newInterval[0] <= curr_end <= newInterval[1])):
